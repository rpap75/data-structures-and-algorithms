...
const raisedToTheThird = (arr) => {
  ...
  // Solution code here...
  arr.forEach(num => {
    const newNumber = Math.pow(num, 3);
...
const forEachTwoToThe = (arr) => {
  ...
  const sameArray = [];
  arr.forEach(num => {
    const raiseTwo = Math.pow(2, num);
    sameArray.push(raiseTwo);
  });
  return sameArray;
};
...
const mapTwoToThe = (arr) => {
  ...
  arr.map(num => {
    const twoAgain = Math.pow(2, num);
    anotherArray.push(twoAgain);
  });
  return anotherArray;
};
